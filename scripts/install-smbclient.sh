...
if ! ${DOCKER_BIN} exec "${CONTAINER}" bash -lc "apt-get update && apt-get install -y --no-install-recommends ${pkgs_base} ${php_pkg} && rm -rf /var/lib/apt/lists/*"; then
  ...
  php_ver=$(${DOCKER_BIN} exec "${CONTAINER}" php -r 'echo PHP_MAJOR_VERSION.".".PHP_MINOR_VERSION;' 2>/dev/null || echo "8.2")
  php_pkg="php${php_ver}-smbclient"
  echo "[info] tentando ${php_pkg}"
  ${DOCKER_BIN} exec "${CONTAINER}" bash -lc "apt-get update && apt-get install -y --no-install-recommends ${pkgs_base} ${php_pkg} && rm -rf /var/lib/apt/lists/*"
fi

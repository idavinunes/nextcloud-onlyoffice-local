...
- Para evitar pegar releases muito novas, o `.env` gerado já define `NEXTCLOUD_IMAGE=nextcloud:30-apache` (linha de suporte atual). Por padrão usamos a imagem oficial; se quiser SMB/SFTP (smbclient + ssh2) use o override `docker-compose.custom.yml` (abaixo) — ele builda `Dockerfile.nc` usando `nextcloud:30-apache-bullseye` como base e compila smbclient/ssh2 via pecl. Rode `docker compose -f docker-compose.yml -f docker-compose.custom.yml build app cron && docker compose -f docker-compose.yml -f docker-compose.custom.yml up -d`. **Não faça downgrade de versão já instalada**.
